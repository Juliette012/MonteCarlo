 ...
 * @author
 */
@CommandLine.Command(name = "MCTS", version = "MCTS 1.0", description = "Solves a specified planning problem using Monte Carlo Tree Search.", sortOptions = false, mixinStandardHelpOptions = true, headerHeading = "Usage:%n", synopsisHeading = "%n", descriptionHeading = "%nDescription:%n%n", parameterListHeading = "%nParameters:%n", optionListHeading = "%nOptions:%n")
public class MCTS extends AbstractPlanner {
    ...
    public Plan solve(final Problem problem) {
        ...
        // StateSpaceSearch.getInstance(SearchStrategy.Name.MCTS,
     ...
     * Launch both MCTS and HSP on all the problems of the blocks, depot, gripper
    ...
    public Plan mcts(Problem problem) {
        ...
        Node n = new Node(init, null, -1, 0, heuristic.estimate(init, problem.getGoal()));
        double hMin = heuristic.estimate(init, problem.getGoal());
        int steps = 0;
        while (!n.satisfy(problem.getGoal())) {

            if ((steps > MAX_STEPS) || (deadEnd(Apossibles(n, problem)))) {
                steps = 0;
                n = new Node(init, null, -1, 0, heuristic.estimate(init, problem.getGoal()));
            }
            n = randomWalk(n, problem);
            double hs = heuristic.estimate(n, problem.getGoal());
            if (hs < hMin) {
                hMin = hs;
                steps = 0;
                // LOGGER.info("hMin : " + hMin);
            } else {
                steps++;
    ...
    public Node randomWalk(Node n, Problem problem) {
        StateHeuristic heuristic = StateHeuristic.getInstance(this.getHeuristic(), problem);
        double hMin = Double.MAX_VALUE;
        Node smin = null;
        for (int k = 0; k < NUM_WALK; k++) {
            Node sPrime = new Node(n, n.getParent(), n.getAction(), 0, heuristic.estimate(n, problem.getGoal()));
            for (int i = 0; i < LENGTH_WALK; i++) {
                Node next = new Node(sPrime);
           
                List<Action> Apossibles = Apossibles(sPrime, problem);
                if (deadEnd(Apossibles)) {
                    break;
                }
                Action a = Apossibles.get((int) (Math.random() * Apossibles.size()));
                //LOGGER.info("Action : " + a.getName());
                ...
                final double g = sPrime.getCost() + 1;
                next.setCost(g);
                ...
                if (next.satisfy(problem.getGoal())) {
                    return next;
                }
                sPrime = next;
            }
        ...
        return smin == null ? n : smin;
    }

    public List<Action> Apossibles(Node n, Problem problem) {
    ...
    public boolean deadEnd(List<Action> Apossibles) {
        return Apossibles.isEmpty();
    }

    public Plan extractPlan(final Node node, final Problem problem) {
        Node n = node;
        ...
        while (n.getAction() != -1) {
            ...
            plan.add(0, a);
            n = n.getParent();
            LOGGER.info("Plan found : " + n.getParent() + " " + n + "`\n");
        }
        return plan;
    }
